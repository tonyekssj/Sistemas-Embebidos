...
#define INC (temperatura++);

static uint8_t estadoLed = 0;
static uint8_t temperatura=10;
...
void comandoTimestamp(uint32_t start){
    ...
    sendPacket(1,cad,sizeof(stop),stop);

}

void comandoEstado(){
    const char cad[]={"11"};
    sendPacket(1,cad,sizeof(estadoLed),estadoLed);
}

void mostrarTemperatura(){
    const char cad[]={"12"};
    sendPacket(1,cad,sizeof(temperatura),temperatura);
}
...
void sendWrongpacket(uart_port_t uart_num){
    ...
    packageUart.command=0;
    packageUart.longitudDato=0;
    packageUart.dato=0;
...
void app_main(void)
{
    uint32_t startTimestamp=xTaskGetTickCount();
    gpio_reset_pin(2);
    gpio_set_direction(2,GPIO_MODE_OUTPUT); //Configuro el LED del board para el comando 13 de invertir LED
    ...
    while(1) 
    {
        ...
        if (checkPackage()){
            ...
            {
            case 10:
                comandoTimestamp(startTimestamp);
                uartPuts(0,"Enviado TimeStamp");
                break;
            case 11:
                comandoEstado();
                uartPuts(0,"Enviado estado de led");
                break;
            case 12:
                INC;
                mostrarTemperatura();
                uartPuts(0,"Enviado temperatura");
                break;
            case 13:
                invertirEstado();
                uartPuts(0,"Invertir estado de led");
                break;
            default:
                sendWrongpacket(1);
                uartPuts(0,"*Comando Invalido*");
                break;
            }
        }
